...
async function main() {
  ...
  try {
    ...
    for (const viewport of viewports) {
      ...
      assert(titleHeroDiceCycle.slice(0, rewardPairCount).every(step => step.dice.every(die => die.shellFilter.includes('drop-shadow') && die.sideContent !== 'none' && die.sideTransform !== 'none' && die.sideBoxShadow.includes('rgba') && die.faceBoxShadow.includes('rgba') && die.afterContent === 'none')), `${viewport.name}: title reward dice should keep visible 3D side/backing depth without the floating shadow smear ${JSON.stringify(titleHeroDiceCycle)}`);
      if (viewport.mobile && viewport.width <= 720) {
        assert(titleHeroDiceCycle.slice(0, rewardPairCount).every(step => step.dice.every(die => die.faceMaskImage === 'none' && die.faceOverflow === 'hidden')), `${viewport.name}: mobile title reward dice should not mask away the external 3D backing/shadow ${JSON.stringify(titleHeroDiceCycle)}`);
      }
    ...
    assert(progressRoundWin.roundWinnerStatusSuppressChaseDie === false && progressRoundWin.statusChaseDieVisible === true && progressRoundWin.statusChaseDieName === rewardNextAfterEleven.name && progressRoundWin.statusChaseDieEffect === rewardNextAfterEleven.effect, `progress round-win copy probe: normal progress wins should still show the lower winner-pill chase die ${JSON.stringify({ rewardNextAfterEleven, progressRoundWin })}`);
    assert(progressRoundWin.statusChaseDieFitsStatus === true, `progress round-win copy probe: winner status chase die should fit inside the status pill ${JSON.stringify(progressRoundWin)}`);
