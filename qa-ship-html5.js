...
async function main() {
  ...
  try {
    ...
    for (const viewport of viewports) {
      ...
      assert(titleHeroDiceCycle.slice(0, rewardPairCount).every(step => step.dice[0].motionClass === 'left' && step.dice[0].animationName === 'startYellowStartled' && step.dice[1].motionClass === 'right' && step.dice[1].animationName === 'startGreenStartled'), `${viewport.name}: title reward dice should keep the default can-bite jump animations ${JSON.stringify(titleHeroDiceCycle)}`);
      assert(titleHeroDiceCycle.slice(0, rewardPairCount).every(step => step.dice.every(die => die.sideContent !== 'none' && die.sideTransform !== 'none' && die.faceBoxShadow.includes('rgba'))), `${viewport.name}: title reward dice should keep 3D side depth like the default hero dice ${JSON.stringify(titleHeroDiceCycle)}`);
