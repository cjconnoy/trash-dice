...
async function main() {
  ...
  try {
    ...
    for (const viewport of viewports) {
      ...
      assert(titleHeroDiceCycle.slice(0, rewardPairCount).every(step => step.dice.every(die => die.sideContent !== 'none' && die.sideTransform !== 'none' && die.faceBoxShadow.includes('rgba') && die.afterContent === 'none')), `${viewport.name}: title reward dice should keep 3D side depth without the floating shadow smear ${JSON.stringify(titleHeroDiceCycle)}`);
